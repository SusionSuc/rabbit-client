...
class ActivitySpeedMonitorTransform : RabbitClassTransformer {
    ...
    override fun onPreTransform(context: TransformContext) {
        ...
        context.artifacts.get(ArtifactManager.MERGED_MANIFESTS).forEach { manifest ->
            RabbitTransformPrinter.p("manifest name : ${manifest.absolutePath}")
        ...
        activityList.forEach {
            RabbitTransformPrinter.p("transform activity name: $it")
        }
    }


    override fun transform(context: TransformContext, klass: ClassNode): ClassNode {
        if (!activityList.contains(klass.className)) {
            return klass
        }
