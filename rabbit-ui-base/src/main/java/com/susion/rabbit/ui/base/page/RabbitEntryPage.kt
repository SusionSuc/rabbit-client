 ...
 * 入口View
 */
class RabbitEntryPage(context: Context, val defaultSupportFeatures:ArrayList<com.susion.rabbit.config.RabbitMainFeatureInfo>) : RabbitBasePage(context) {

    private val rv = RecyclerView(context)
    private val featuresAdapter by lazy {
        object : RabbitRvAdapter<com.susion.rabbit.config.RabbitMainFeatureInfo>(defaultSupportFeatures) {
            override fun createItem(type: Int) =
                RabbitMainFeatureView(context)
            override fun getItemType(data: com.susion.rabbit.config.RabbitMainFeatureInfo) = 0
        }
    }
