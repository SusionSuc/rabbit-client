...
object RabbitDbStorageManager {
    ...
    fun save(obj: Any, reportData: Boolean = true, reportTime: Long = System.currentTimeMillis()) {
        val dis = runOnDbThread({
            greenDaoDbManage.saveObj(obj)
        })
        disposableList.add(dis)
        if (reportData) {
            RabbitDataReportCenter.report(obj, reportTime)
        }
    }

    fun saveSync(
        obj: Any,
        reportData: Boolean = true,
        reportTime: Long = System.currentTimeMillis()
    ) {
        greenDaoDbManage.saveObj(obj)
        if (reportData) {
            RabbitDataReportCenter.report(obj, reportTime)
